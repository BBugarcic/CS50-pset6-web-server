...
int main(int argc, char* argv[])
{
    ...
    while (true)
    {
        ...
        if (connected())
        {
            ...
            // devide line in tokens(strings): method, request target and http-version
            // extracts tokens from the line
            char* token = strtok(line, " ");
            
            // declaring an array to place in the first token (method)
            // fixed size is the length of the first token
            char method[strlen(token) + 1];
                       
            // coping the first token to method array
            memset(method,'\0', sizeof(method));
            strcpy(method, token);
            
            // second token (request-target)
            token = strtok(NULL, " ");
            ...
            token = strtok(NULL, " ");
            int httpcrlfLen = strlen(token);
            
            char httpv[httpcrlfLen - 1];
            memset(httpv, '\0', sizeof(httpv));
            strncpy(httpv, token, 8);
                    
            // check if request-target is valid
            // if it cointain more than two spaces
            if (strtok(NULL, " ") != NULL)
                error(400);
            
            // if the first character in request-target is not '/'
            if (request_target[0] != '/')
                error(501);
            ...
            // if apsoluth-path does not contains file extension (.)
            const char p = '.';
            ...
            if (qmquery != 0)
            {
                // if there is no query after '?'
                if (strcmp(qmquery, "?") == 0)
                {
                    // buffer for storing query
                    char q_buffer[1];
                    q_buffer[0] = '\0';
                    strcpy(query, q_buffer);               
                }
                // if there is a query after '?'
                else
                {
            ...
            // length of the request-target
            int rtlen = strlen(request_target);
            
            int qlen;
            // length of the query (including ?)
            if (qmquery != NULL)
                qlen = strlen(qmquery);
            else
                qlen = 0; 
            
            // length of absolute-path with out '\0' at the end
            ...
            // concatenate root and abspath
            char path[strlen(pathnoabs) + strlen(abspath) + 1];
            memset(path, '\0', sizeof(path));
            strcpy(path, pathnoabs);
            strcat(path, abspath);
            ...
            // extract path's extension including '.'
            char* dotextension;
